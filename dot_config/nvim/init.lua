...
-- Neat clipboard integration for both Linux and macOS
-- <leader>p will paste clipboard into buffer
-- <leader>c will copy entire buffer into clipboard
-- Detect the operating system
local is_macos = vim.fn.has("macunix") == 1
-- Set commands based on the operating system
local paste_cmd = is_macos and 'pbpaste' or 'wl-paste'
local copy_cmd = is_macos and 'pbcopy' or 'wl-copy'
vim.keymap.set('n', '<leader>p', '<cmd>read !' .. paste_cmd .. '<cr>')
vim.keymap.set('n', '<leader>c', '<cmd>w !' .. copy_cmd .. '<cr><cr>')

-- 绑定 <leader>y 复制选定内容到剪贴板
vim.keymap.set('v', '<leader>y', '"+y')
-- if nvim no internal support clipboard, show info :echo has('clipboard')
-- Linux 使用 xclip
-- vim.keymap.set('v', '<leader>y', ':w !xclip -selection clipboard<CR><CR>', { silent = true })
-- macOS 使用 pbcopy
-- vim.keymap.set('v', '<leader>y', ':w !pbcopy<CR><CR>', { silent = true })
